...
class ReCogDriveFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        if not self.cache_hidden_state:
            ...
            #     - if not, regenerate entirely
            #     - do not output invalid trajectory
            #     """

            # # 🔥 CHÈN teacher vào giữa
            # questions = [f"{prompt}\n{teacher}\n{output_requirements}"]
            # print(f"==>> questions: {questions}")

            outputs = self.backbone(pixel_values_cat.cuda(), questions, num_patches_list=num_patches_list)
            last_hidden_state = outputs.hidden_states[-1]
            # last_hidden_state = torch.nan_to_num(outputs.hidden_states[-1], nan=0.0, posinf=0.0, neginf=0.0)
            ...
            #     temperature=0.7,
            # )
            # param = next(self.backbone.model.parameters())
            # pixel_values_cat = pixel_values_cat.to(device=param.device, dtype=param.dtype)

            # response = self.backbone.model.chat(
