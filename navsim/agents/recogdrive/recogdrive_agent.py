...
class ReCogDriveAgent(AbstractAgent):
    def __init__(
        ...
        feedback_lora_path: Optional[str] = '',
        failure_threshold: float = 0.8,
        num_refined_samples: int = 4,
        teacher_server_port: int = 0,   # >0 → use TeacherModelClient (multi-GPU mode)
    ):
        super().__init__()
        ...
        # ELF-VLA: teacher model or client
        self.teacher_model = None
        if teacher_server_port > 0:
            # Multi-GPU mode: query external teacher_server.py via socket
            from navsim.agents.recogdrive.teacher_interface import TeacherModelClient
            self.teacher_model = TeacherModelClient(host="localhost", port=teacher_server_port)
            print(f"[Agent] Using TeacherModelClient → localhost:{teacher_server_port}")
        elif teacher_model_path:
            # Single-GPU mode: load teacher directly on cuda:1
            from navsim.agents.recogdrive.teacher_interface import TeacherModel
            self.teacher_model = TeacherModel(teacher_model_path, device="cuda:1")

        self.feedback_backbone = None
        if feedback_lora_path:
            _fb_local_rank = int(os.getenv("LOCAL_RANK", "0"))
            self.feedback_backbone = RecogDriveBackbone(
                model_type="internvl",
                checkpoint_path=feedback_lora_path,
                device=f"cuda:{_fb_local_rank}",
            )
            for p in self.feedback_backbone.parameters():
                p.requires_grad = False

        # --- [Device Check] -----------------------------------------------
        if self.teacher_model is not None or self.feedback_backbone is not None:
            print("\n[Device Check]")
            if self.teacher_model is not None:
                from navsim.agents.recogdrive.teacher_interface import TeacherModel
                if isinstance(self.teacher_model, TeacherModel):
                    _t_dev = next(self.teacher_model.model.parameters()).device
                    print(f"teacher_device={_t_dev}")
                else:
                    print(f"teacher_device=remote (socket client)")
            else:
                print("teacher_device=None (not loaded)")
            if self.feedback_backbone is not None:
                _f_dev = next(self.feedback_backbone.parameters()).device
                print(f"feedback_lora_device={_f_dev}")
            else:
                print("feedback_lora_device=None (not loaded)")
        # ------------------------------------------------------------------
        ...
        if self.grpo:
            ...
            cfg.grpo_cfg.failure_threshold   = failure_threshold
            cfg.grpo_cfg.num_refined_samples = num_refined_samples
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets=None, tokens_list=None) -> Dict[str, torch.Tensor]:
        ...
        if self.training and not self.grpo:
            ...
            if image_path_tensor is not None and image_path_tensor.ndim == 1:
                image_path_tensor = image_path_tensor.unsqueeze(0)
            return self.action_head.forward_grpo(
                ...
                feedback_backbone=self.feedback_backbone,
            )
        else: 
