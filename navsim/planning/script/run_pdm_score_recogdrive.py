    ...
    if dist.get_rank() == 0:
        ...
        logger.info(
            ...
            Finished running evaluation.
                ...
                NC↑  (no_at_fault_collisions):        {valid_df['no_at_fault_collisions'].mean():.4f}
                DAC↑ (drivable_area_compliance):       {valid_df['drivable_area_compliance'].mean():.4f}
                TTC↑ (time_to_collision_within_bound): {valid_df['time_to_collision_within_bound'].mean():.4f}
                CF↑  (comfort):                        {valid_df['comfort'].mean():.4f}
                EP↑  (ego_progress):                   {valid_df['ego_progress'].mean():.4f}
                DDC↑ (driving_direction_compliance):   {valid_df['driving_direction_compliance'].mean():.4f}
                PDMS↑ (score):                         {valid_df['score'].mean():.4f}
